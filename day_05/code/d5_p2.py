...
def create_X_Y_deafultdict(input_list):
    x_y_Dict: dict = defaultdict(
        lambda: [[], []])  # create a defultdict with an [[],[]] empty 2d list as the defult value
...
def check_after_X(token_location, x_y_pair, row):
    ...
    for i in range(token_location + 1, row_length):  # for each item from the token to the end
        ...
        find_value = row[i]
        bool_value = find_value in x_y_pair[token][
            1]  # check if the item exists in the "after list" inside the x_y_defultdict
...
def check_before_X(token_location, x_y_pair, row):
    ...
    for i in range(0, token_location):  # for each item from the beginning to the token
        ...
        bool_value = find_value in x_y_pair[token][
            0]  # check if the item exists in the "before list" inside the x_y_defultdict
        if bool_value == False:
            return 0
        else:
            continue
...
def swap_list_pair(first, second,
                   row):  # takes in a list and two of its items, then swiches their places with each other before retruning the new list
...
def recheck_pair(iteration, token, x_y_pair, item,
                 compere_token):  # returns a boolen value depending on it is check the items before or after the token
